...
/* Platform winding orientation for USER FBOs, derived from the GPU by
 * front_face_regression(): 0 = desktop GL semantics (GL_CCW quad is front
 * under a positive-height viewport), 1 = iOS surface semantics (window-space
 * winding reverses on user FBOs too; see prepare_draw's TARGET_OS_IPHONE
 * branch). All winding-sensitive expectations below key off this flag so the
 * contract (front/back asymmetry) is asserted on both orientations. */
static int g_user_fbo_winding_inverted = 0;
...
static void front_face_regression(Api* api, GLuint fbo, GLuint vao, GLuint ebo,
    ...
    unsigned char px[4];
    read_center(api, px);
    g_user_fbo_winding_inverted = nonblack(px) ? 0 : 1;
    printf("info: user-FBO winding orientation: %s\n",
           g_user_fbo_winding_inverted ? "inverted (iOS surface)" : "GL (desktop)");
    CHECK(nonblack(px) == !g_user_fbo_winding_inverted,
          "user FBO GL_CCW + GL_BACK visibility matches platform winding "
    ...
    read_center(api, px);
    CHECK(nonblack(px) == !g_user_fbo_winding_inverted,
          "user FBO GL_CW + GL_BACK visibility matches platform winding "
          "(rgba=%u,%u,%u,%u)", px[0], px[1], px[2], px[3]);
    ...
    read_center(api, px);
    CHECK(nonblack(px) == g_user_fbo_winding_inverted,
          "user FBO GL_CCW + GL_FRONT culls exactly the opposite face "
          "(rgba=%u,%u,%u,%u)", px[0], px[1], px[2], px[3]);
    api->cullFace(GL_BACK);
}
...
static void logical_projection_front_face_regression(
    ...
    read_center(api, px);
    CHECK(nonblack(px) == !g_user_fbo_winding_inverted,
          "logical GUI projection + GL_CW + GL_BACK visibility matches platform winding "
          "(rgba=%u,%u,%u,%u)", px[0], px[1], px[2], px[3]);
    ...
    read_center(api, px);
    CHECK(nonblack(px) == g_user_fbo_winding_inverted,
          "logical GUI projection + GL_CCW + GL_BACK culls the opposite face "
          "(rgba=%u,%u,%u,%u)", px[0], px[1], px[2], px[3]);
    ...
    read_center(api, px);
    CHECK(nonblack(px) == !g_user_fbo_winding_inverted,
          "logical GUI projection + GL_CCW + GL_FRONT visibility matches platform winding "
          "(rgba=%u,%u,%u,%u)", px[0], px[1], px[2], px[3]);
    api->cullFace(GL_BACK);
...
static void gui_regression(Api* api, GLuint fbo, GLuint vao, GLuint ebo,
    ...
    api->scissor(0, 0, 378, 248);
    api->enable(GL_SCISSOR_TEST);
    /* This section asserts GUI visibility/blending/descriptor identity, not
     * winding; culling would make the result depend on the platform
     * orientation probed above. */
    api->disable(GL_CULL_FACE);
    api->disable(GL_DEPTH_TEST);
    api->enable(GL_BLEND);
...
static void descriptor_stress(Api* api, GLuint fbo, GLuint vao, GLuint gui_program,
    ...
    api->disable(GL_CULL_FACE);  /* descriptor/fence lifecycle, not winding */
