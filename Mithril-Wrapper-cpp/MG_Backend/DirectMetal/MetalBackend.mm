...
int dmt_read_pixels(int x, int y, int w, int h,
    ...
    if (!g_state || g_state->currentReadFBO == 0) {
        // Default FBO exposes separate opaque cookies for color and depth.
        // Select the attachment from the requested GL read format instead of
        // always reading color0 (depth-only readback must use the depth image).
