...
VkBuffer backend_get_or_create_buffer(GLuint name, const void* data, size_t size) {
    ...
    if (it != tbl.end() && it->second.size >= (VkDeviceSize)size) {
        // FIX (GPU Address Fault 根因 — orphan recycling 风暴):
        // MC 每帧对同一 buffer 调 glBufferData 上传相同大小数据。原实现
        // 每次 orphan 都按请求大小 create 新 VkBuffer + defer 旧 buffer →
        // 即使大小未变也反复 alloc/free（每帧 20+ 次 per buffer × 多个 buffer）。
        // 这导致 GPU 显存碎片化 + Metal argument buffer 分配器压力 →
        // MoltenVK "GPU Address Fault Error" → DEVICE_LOST 红屏。
        //
        // 修复策略（参照 MobileGL VkBufferManager 的 busy 处理 + 三缓冲思想）:
        // 当 buffer 已存在且容量足够时，【绝不】走 orphan 路径。直接原地更新
        // (map + memcpy + unmap)。理由：
        //   1. 请求大小 ≤ 已分配大小 → 数据完全容纳，无溢出风险
        //   2. 撕裂风险极低：MC 每帧完整重写 buffer（非部分更新），GPU 要么
        //     读到旧帧完整数据要么读到新帧完整数据，不存在"半新半旧"
        //   3. 彻底消除 alloc/free 循环 → 零碎片化 → 零 DEVICE_LOST
        // 仅当 buffer 容量不足（size > it->second.size）时才走下方 grow 路径
        // （先 create 新 buffer 再 defer 旧的，保证不返回 NULL）。
        if (data && size > 0) {
            void* dst = nullptr;
