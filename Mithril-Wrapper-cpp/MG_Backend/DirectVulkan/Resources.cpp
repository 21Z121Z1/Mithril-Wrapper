...
VkBuffer backend_get_or_create_buffer(GLuint name, const void* data, size_t size) {
    ...
    if (it != tbl.end() && it->second.size >= (VkDeviceSize)size) {
        ...
        // Persistently mapped storage must stay mapped for its whole lifetime;
        // re-mapping it here is invalid and was observed as a device-fault path
        // during Minecraft's high-frequency upload workload on iOS.
        if (data && size > 0) {
