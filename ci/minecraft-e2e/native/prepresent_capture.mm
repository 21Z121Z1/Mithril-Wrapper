...
extern "C" void mithril_e2e_capture_before_present(int width, int height, void* mithril_handle) {
    ...
    // which call produced it. Minecraft can legitimately leave an unrelated GL
    // error queued before this test-only seam runs. The previous oracle read
    // glGetError only AFTER glReadPixels and therefore misattributed any stale
    // error to the readback itself. Drain and record pre-existing errors first,
    // then separately gate errors generated by the controlled capture sequence.
    auto drain_errors = [&](const char* event) -> GLenum {
        GLenum first = GL_NO_ERROR;
        GLenum error = GL_NO_ERROR;
        int count = 0;
        while (count < 32 && (error = getError()) != GL_NO_ERROR) {
