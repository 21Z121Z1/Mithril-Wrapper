...
| Default framebuffer size/present | partial | `amethyst_egl_smoke` same-command-buffer BGRA drawable capture | `eglSwapBuffers` submits a pending clear without an explicit GL flush, converts the RGBA default target into a BGRA drawable, and synchronizes non-square `drawableSize` changes before acquiring the replacement drawable. The test-only capture occurs before `presentDrawable` on the presentation command buffer because post-present contents are not a portable readback contract. Logical size vs physical pixels under UIKit scale, visible WindowServer/real-device presentation, nil drawable recovery, and surface replacement still need a standalone lifecycle app. |
