...
| Depth/stencil/blend/cull/polygon | partial | `fbo_smoke` pixel assertions | Basic depth, stencil, blend, cull, front-face and line polygon mode execute. Blend enum validation, independent indexed blend/masks, point polygon mode, and several uncommon factors are incomplete or rejected. |
...
| FBO / MRT / MSAA / subresources | partial | `fbo_smoke`, `directmetal_fbo_smoke` | Color/depth/renderbuffer FBOs, MRT, 4x resolve, mip selection, 2D-array slice and 3D depth-plane readback execute. Scaled/filtered, depth/stencil and multisample blits plus multisample array/depth textures are unsupported. |
