...
> 状态：**M5 进行中（stage A+B 完成）**；M4 及以前完成（Linux 六冒烟测试通过）。
> 配套文档：`docs/gl33_core_list.md`（GL 3.3 core 342 函数分组）、`docs/egl_list.md`（EGL 符号清单）。
> 测试：`tests/contract_smoke.c`（EGL 契约）、`tests/state_smoke.c`（GL 状态机）、`tests/shader_smoke.c`（着色器管线）、`tests/draw_smoke.c`（GL→Vulkan→读回全链）、`tests/texture_smoke.c`（M4 纹理全链）、`tests/fbo_smoke.c`（M5 状态管线：depth/scissor/blend/cull/stencil/colorMask/polygon），均需 lavapipe/llvmpipe。
...
> **M5 进行中（stage A+B 完成）**：状态管线接入 Vulkan——深度附件（D24S8，renderpass 第二附件 + framebuffer 挂载）、`PipelineState` 快照（GL->vk 枚举映射 ToVkCompare/ToVkBlend/StencilOp/ColorMask）烘焙进 pipeline 缓存 key（`StateSignature`，含 depth/blend/scissor/cull/frontFace/stencil 全域含读/写掩码与 ref）与 `VkPipelineDepthStencil/ColorBlend/Rasterization...State`；深度清除统一为显式 `CmdClearDepthStencilImage`（depth 附件 loadOp=LOAD，修掉原先 `LOAD_OP_CLEAR` 无 clearValue 把 depth 清 0 的 bug）；动态 scissor 每 draw 下发（Y 翻转 + clamp，修 GL 左下/Vulkan 左上原点差）；`glClear(mask)`/`glDepthFunc`/`glDepthMask`/`glBlendFunc`/`glScissor`/`glColorMask`/`glCullFace`/`glFrontFace`/`glStencilFunc(Op/Mask)`/`glPolygonMode` 接入后端。**fbo_smoke 18 行 ok（17 状态断言）**：depth 近者胜、LEQUAL 同 z 保持、scissor 中心清屏+角落收色、blend 关闭/开 alpha 混合（参考 mobilegl 非预乘）、cull GL_BACK 保留 CCW + GL_FRONT/GL_CW 位翻转剔除、stencil REPLACE 标记 255 → EQUAL 读回 → ref 不匹配被拦、colorMask 全关/R-only/恢复、polygonMode GL_LINE 内部留空 + FILL 恢复。stage B 附带修复：`glStencilMask` 独立写掩码字段（原先与 `glStencilFunc` 读掩码共用、VK writeMask 写死 0xFFFFFFFF）、depth image view 补 S8 aspect、renderpass stencil loadOp=LOAD（原先 DONT_CARE）、stencil ref 入 pipeline key（原先缺 → 换 func/ref 不重建管线）、GL→VK frontFace 取反（VK framebuffer +Y 向下）。回归：五个旧冒烟全过，M3/M4 未破坏；CI Linux job 已接 fbo_smoke。
> 待办：S5 FBO/渲染缓冲（24 函数）+ MRT → MSAA；M6 同步与多帧。
...
| **M5 状态与管线** | depth/stencil/blend/cull/FBO/MSAA → pipeline 缓存 | 完整三维画面 | 🔶 stage A+B 完成（状态管线+depth/scissor/blend/cull/stencil/colorMask/polygon）；余 S5 FBO/MRT/MSAA 验收 |
