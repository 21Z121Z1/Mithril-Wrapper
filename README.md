...
- **DirectMetal 当前诚实边界**：window surface 必须由调用方提供 `CAMetalLayer`，drawable 尺寸改变后的自动重建尚未接通；compute、缩放/过滤 framebuffer blit，以及 3D texture 的 mip 链采样也尚未接通。相关路径明确失败并记录 unsupported，不宣称这些能力。需要这些功能时应显式使用 Vulkan backend。
