...
- **Amethyst 呈现像素契约**：`amethyst_egl_smoke` 使用可捕获的真实 `CAMetalLayer` drawable，证明仅调用 `eglSwapBuffers` 就会提交 pending clear、RGBA default target 会以正确通道写入 BGRA8 drawable，并在 80×48 → 56×96 resize 后对 replacement drawable 复验尺寸和像素。测试构建在同一 presentation command buffer 内、`presentDrawable` 前读回实际 target，避免依赖呈现后 drawable 的复用时机，也不把 `EGL_TRUE` 当成视觉完成。
