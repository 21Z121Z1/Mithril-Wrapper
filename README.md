...
- **Amethyst 呈现像素契约**：`amethyst_egl_smoke` 使用可捕获的真实 `CAMetalLayer` drawable，证明仅调用 `eglSwapBuffers` 就会提交 pending clear、RGBA default target 会以正确通道写入 BGRA8 drawable，并在 80×48 → 56×96 resize 后对 replacement drawable 复验尺寸和像素。测试构建在同一 presentation command buffer 内：实体 Apple GPU 要求真实 drawable 和同 pipeline BGRA reference 的字节都正确；Apple Paravirtual 明确报告 drawable byte-readback capability skip，但 reference 仍是必过 gate。这不把 `EGL_TRUE` 当成视觉完成，也不把 hosted 虚拟 drawable 当成真机显示证据。
