...
- **Amethyst 呈现像素契约**：`amethyst_egl_smoke` 使用可捕获的真实 `CAMetalLayer` drawable，证明仅调用 `eglSwapBuffers` 就会提交 pending clear、RGBA default target 会以正确通道写入 BGRA8 drawable，并在 80×48 → 56×96 resize 后对 replacement drawable 复验尺寸和像素。测试读取的是实际 presentation target，不把 `EGL_TRUE` 当成视觉完成。
