...
- **Amethyst 呈现像素契约**：`eglSwapBuffers` 现在把 pending GL frame 与 RGBA→BGRA presentation encoder 编入同一 Metal command buffer，消除每帧多一次 submit 以及 tile/virtual GPU 上的跨 command-buffer producer/consumer 间隙。`amethyst_egl_smoke` 使用可捕获的真实 `CAMetalLayer` drawable，证明仅调用 swap 就会提交 pending clear，并在 80×48 → 56×96 resize 后复验 replacement drawable。实体 Apple GPU 要求真实 drawable 和同 pipeline BGRA reference 的字节都正确；Apple Paravirtual 只有在 reference 通过且 drawable 全零时才明确报告 byte-readback capability skip。这不把 `EGL_TRUE` 当成视觉完成，也不把 hosted 虚拟 drawable 当成真机显示证据。
